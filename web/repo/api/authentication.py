...
class CustomOpenRepoPermission(permissions.BasePermission):
    ...
    def has_object_permission(self, request, view, obj):
        ...
        logger.debug(
            f"Object permission test for {request.user.username} / {view.__class__.__name__} obj {obj.__class__.__name__}"
        )
        # Read permissions are allowed to any request,
        # so we'll always allow GET, HEAD or OPTIONS requests.
        if request.method in permissions.SAFE_METHODS:
        ...
        if isinstance(obj, Repository):
            ...
            logger.debug(
                f"Package write attempt by {request.user.username} on repo {obj.repo.repo_uid} for pkg {obj.package_uid}"
            )
            return self._check_perm(request.user, obj.repo)

        logger.debug(
            f"Obj Access denied for user {request.user} -- view: {view.__class__.__name__} obj: {obj.__class__.__name__}"
        )
        return False
